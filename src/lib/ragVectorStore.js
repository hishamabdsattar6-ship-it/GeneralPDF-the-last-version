 ...
 */
export async function vectorizeAndStore(text, fileId) {
 ...
 */
export async function searchInVectorDB(question, fileId, topK = 5) {
    ...
    const retrievedContexts = queryResponse.matches?.map(match => match.metadata?.text) || [];
